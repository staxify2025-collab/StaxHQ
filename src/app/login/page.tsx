...
export default function LoginPage() {
  ...
  const { login, resetTeamMemberPassword, teamMembers } = useTenant();
  ...
  const handlePasswordReset = (e: React.FormEvent) => {
    ...
    const trimmedEmail = email.trim();
    if (!trimmedEmail) {
      setError("Please enter your registered team email address to reset password.");
      return;
    }
    ...
    try {
      // 1. Reset password in state and persistence
      resetTeamMemberPassword(trimmedEmail, newPassword);

      // 2. Automatically log the user in with their newly set password
      const result = login(trimmedEmail, newPassword);

      if (result.success) {
        setSuccessMessage(`Password successfully reset! Signing in as ${trimmedEmail}...`);
        setTimeout(() => {
          router.push("/dashboard");
        }, 600);
      } else {
      ...
      setError("Failed to reset password. Please try again or contact system administration.");
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen w-full bg-slate-950 text-slate-100 flex flex-col justify-center items-center px-4 py-12 relative overflow-hidden selection:bg-indigo-500 selection:text-white">
      ...
      <div className="w-full max-w-md space-y-6 relative z-10">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/80 backdrop-blur-xl p-7 shadow-2xl space-y-5">
          ...
          {error && (
            <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs font-medium flex flex-col gap-2 animate-in fade-in-50">
              ...
              {mode === "signin" && (
                ...
                >
                  Forgot or need to change password? Reset it here ➔
                </button>
              )}
            </div>
          )}
          ...
          {mode === "signin" ? (
            ...
            <form onSubmit={handlePasswordReset} className="space-y-4">
              <div className="bg-indigo-950/40 border border-indigo-500/20 rounded-xl p-3 text-[11px] text-indigo-300">
                <span className="font-semibold text-indigo-200">Password Recovery:</span> Enter your team email address and choose a new password. You will be authenticated immediately.
              </div>
              ...
              <div className="space-y-2 pt-1">
                ...
                >
                  ...
                  <span>{isLoading ? "Updating Password..." : "Set New Password & Sign In"}</span>
                </Button>
        ...
        {/* Quick Access & Recovery Drawer Toggle */}
        <div className="rounded-xl border border-slate-800/80 bg-slate-900/50 backdrop-blur-md p-3.5 space-y-2.5">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span>Quick Authorized Access & 1-Click Sign In</span>
            </div>
          ...
          {showQuickAccess && (
            <div className="space-y-2 pt-2 border-t border-slate-800 animate-in fade-in-50">
              <p className="text-[11px] text-slate-400">
                Click any profile to instantly sign in with executive credentials:
              </p>
              ...
              <div className="grid grid-cols-1 gap-1.5">
                ...
                <button
                  type="button"
                  onClick={() => handleQuickLogin("admin@staxhq.com", "admin123")}
                  className="flex items-center justify-between p-2 rounded-lg bg-slate-950/60 hover:bg-slate-800/80 border border-slate-800 text-left transition-all group"
                >
                  <div className="flex items-center gap-2">
